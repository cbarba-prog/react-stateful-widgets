...
export default function Programmers() {
  const [programName, setProgramName] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    return programName[programId -1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programId != null ?  'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programName.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => { setProgramID(dev.id)/* set the featured id passing dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
